...
// Verify a Gumroad license key via the Gumroad API
  ...
  try {
    const response = await fetch('https://api.gumroad.com/v2/licenses/verify', {
      ...
      body: new URLSearchParams({
        product_permalink: process.env.GUMROAD_PRODUCT_PERMALINK || 'cfevjo',
...
const activate = async (req, res) => {
  try {
    ...
    if (!licenseKey || licenseKey.trim().length < 10) {
      return res.status(400).json({ success: false, message: 'Please provide a valid license key.' });
    }
    ...
    // Ensure the key isn't already tied to another account
...
module.exports = { getStatus, activate, adminActivate, cancel };
